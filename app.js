...
function shouldEmitImportantNotification(targetRoles, targetArea, message, noSurat = '') {
  ...
  const importantPatterns = ['PERMINTAAN BARU', 'DISETUJUI SERVICE', 'APPROVAL DM', 'MOHON APPROVAL DM', 'DITOLAK', 'SELESAI (DONE)', 'REMINDER PENDING'];
  ...
  if (noSuratKey && noSuratKey.startsWith('PRMT/')) return true;
  return true;
}

function tambahNotifikasiSistem(targetRoles, targetArea, message, noSurat = '') {
  if (!shouldEmitImportantNotification(targetRoles, targetArea, message, noSurat)) return;
  ...
  const alreadyExists = notifs.some(n => `${String(n.noSurat || '').trim()}|${String(n.targetArea || 'ALL')}|${String(n.message || '').trim()}` === dedupeKey);
...
function getAccessibleNotifications() {
  ...
  return notifs.filter(n => {
    ...
    const roleMatch = (n.targetRoles.includes('ALL') || n.targetRoles.includes(currentUser.category) || (currentUser.category === 'TOKO' && n.targetRoles.includes('TOKO')));
...
function bukaNotificationModal() {
  ...
  else if (typeof loadNotificationList === 'function') loadNotificationList(); 
  
  const listBody = document.getElementById('notifListBody');
  if (listBody && listBody.innerHTML.trim() === '') {
    listBody.innerHTML = `<div style="padding: 20px; text-align: center; color: var(--text-muted); font-size: 12px; font-weight: bold;">TIDAK ADA NOTIFIKASI SAAT INI.</div>`;
  }
...
function loadNotificationList() {
  ...
  userNotifs.forEach(n => {
    ...
    item.style.cssText = `padding: 12px; margin-bottom: 8px; border-radius: 8px; border: 1px solid var(--border-color); background: ${isRead ? 'var(--bg-box)' : 'var(--bg-header)'}; cursor: pointer; display: flex; gap: 12px; align-items: flex-start; transition: background 0.2s;`;
    ...
    item.innerHTML = `
      ...
      <div style="flex: 1;">
        <div style="font-size: 12.5px; font-weight: ${isRead ? '500' : '700'}; color: var(--text-main); line-height: 1.4;">${n.message}</div>
...
function clickNotificationItem(notifId, noSurat) {
  ...
  if (noSurat) {
    setTimeout(() => { lihatDetail(noSurat, true); }, 150);
  }
}
...
function generateStoreCode(namaToko) {
  ...
  else if (words.length === 1 && words[0].length >= 2) return words[0].substring(0, 2).toUpperCase();
  else {
...
function getStoresFromDB() {
  ...
  const userStores = users.filter(u => u && u.category === 'TOKO').map(u => ({
    id: u.id, fullName: u.fullName || 'TOKO', area: u.area || '', storeCode: u.storeCode || generateStoreCode(u.fullName || '')
  }));
  const map = new Map();
...
// =========================================================================
// BOOT SEQUENCE (KONEKSI INSTAN & WAJIB TUNGGU SUPABASE UNTUK CEGAH BLANK)
// =========================================================================
document.addEventListener('DOMContentLoaded', async () => {
  try {
    closeAllPopups();
    
    showLoading('MENGUNDUH DATA DARI SUPABASE...');

    if (typeof initSupabaseDB === 'function') {
      const isConnected = await initSupabaseDB();
      if (!isConnected) {
        showNotif('GAGAL TERHUBUNG KE CLOUD SUPABASE. PERIKSA INTERNET!', 'error');
      }
    }
...
function initPullToRefresh() {
  ...
  let startY = 0, moveY = 0, isAtTop = false;
  container.addEventListener('touchstart', (e) => {
    if (container.scrollTop <= 5) { startY = e.touches[0].clientY; isAtTop = true; } else { isAtTop = false; }
  }, { passive: true });
  ...
  container.addEventListener('touchend', async () => {
    ...
    if (dist > 80 && container.scrollTop <= 5) { await pullCentralCloudDB(); }
...
function checkAndTriggerPendingReminders() {
  ...
  if (pendingServiceReqs.length > 0) {
    pendingServiceReqs.forEach(r => {
      ...
      if (!notifs.some(n => n.noSurat === r.noSurat && String(n.message).includes('REMINDER PENDING') && String(n.message).includes('SERVICE'))) {
  ...
  if (pendingDMReqs.length > 0) {
    pendingDMReqs.forEach(r => {
      ...
      if (!notifs.some(n => n.noSurat === r.noSurat && String(n.message).includes('REMINDER PENDING') && String(n.message).includes('DM'))) {
...
function onSupabaseDataChange(keyChanged) {
  ...
  if (popupBantuan && (popupBantuan.classList.contains('show') || popupBantuan.style.display === 'block')) {
    if (typeof isAdminChat !== 'undefined' && isAdminChat) {
      if (typeof currentRoom !== 'undefined' && currentRoom && typeof loadChatAdmin === 'function') loadChatAdmin(currentRoom);
      else if (typeof loadDaftarChatAdmin === 'function') loadDaftarChatAdmin();
    } else {
...
function startCentralCloudSyncEngine() {
  if (typeof setOnDataChangeCallback === 'function') setOnDataChangeCallback(onSupabaseDataChange);
}

async function pushCentralCloudDB() { if (typeof pushToSupabaseNow === 'function') await pushToSupabaseNow(); }
...
function loadSavedTheme() {
  const saved = (window.localStorage ? window.localStorage.getItem(THEME_KEY) : null) || 'dark-mode';
...
function toggleTheme() {
  ...
  if (window.localStorage) window.localStorage.setItem(THEME_KEY, t.id);
  updateThemeIcon();
}
...
// =========================================================================
// AUTO LOGIN & PROSES LOGIN (MEMAKAI SESSION STORAGE LOKAL & RAM)
// =========================================================================
function autoLogin() {
  const savedSession = sessionStorage.getItem(SESSION_KEY);
  if (savedSession) {
...
async function prosesLogin() {
  ...
  try {
    if (!supabaseClient && typeof initSupabaseDB === 'function') {
...
async function catatLogLogin(username, nama, area, status) {
  ...
  try {
    await supabaseClient.from('log_login').insert([{ username: username, nama_lengkap: nama, area: area, status: status }]);
  } catch (e) {
    console.warn('Gagal mencatat log login.');
  }
}
...
// =========================================================================
// PERBAIKAN FORM AGAR TIDAK RESET SAAT REALTIME SYNC (KEYBOARD AMAN)
// =========================================================================
function loadForm() {
...
function gantiFotoViewer(arah) {
  ...
  if (viewerCurrentIndex < 0) viewerCurrentIndex = viewerPhotos.length - 1;
  else if (viewerCurrentIndex >= viewerPhotos.length) viewerCurrentIndex = 0;
  tampilkanFotoViewerAktif();
}
  ...
  showConfirm(`APPROVE PERMINTAAN?`, () => {
    ...
    setTimeout(() => {
      ...
      if (idx !== -1) {
        ...
        if (sig) requests[idx].serviceTTD = sig;

        if (!requests[idx].log) requests[idx].log = [];
        requests[idx].log.push({ action: 'APPROVE_SERVICE', user: currentUser.fullName, notes: 'DISETUJUI SERVICE', time: `${getFormattedDateDDMMYYYY()} ${new Date().toLocaleTimeString('id-ID')}` });
        saveRequestsToDB(requests);
        ...
        dmUsers.forEach(dm => { if (dm.phone) kirimNotifikasiWA(dm.phone, `PERMINTAAN #${noSurat} DARI ${requests[idx].toko} TELAH DISETUJUI SERVICE (${currentUser.fullName}). MOHON APPROVAL DM.`); });
  ...
  showConfirm(`APPROVE PERMINTAAN #${noSurat}?`, () => {
    ...
    setTimeout(() => {
      ...
      if (idx !== -1) {
        ...
        if (sig) requests[idx].dmTTD = sig;

        if (!requests[idx].log) requests[idx].log = [];
        requests[idx].log.push({ action: 'APPROVE_DM', user: currentUser.fullName, notes: 'DISETUJUI DM PUSAT', time: `${getFormattedDateDDMMYYYY()} ${new Date().toLocaleTimeString('id-ID')}` });
        saveRequestsToDB(requests);
        ...
        serviceUsers.forEach(srv => {
          if (srv.phone) kirimNotifikasiWA(srv.phone, `PERMINTAAN #${noSurat} DARI ${requests[idx].toko} TELAH DISETUJUI DM PUSAT. SILAKAN DIPROSES.`);
        });
  ...
  showConfirm(`UBAH STATUS PERMINTAAN #${noSurat} MENJADI DONE?`, () => {
    ...
    setTimeout(() => {
      ...
      if (idx !== -1) {
        ...
        if (!requests[idx].log) requests[idx].log = [];
        requests[idx].log.push({ action: 'DONE', user: currentUser.fullName, notes: 'BARANG TELAH DISERAHKAN / SELESAI', time: `${getFormattedDateDDMMYYYY()} ${new Date().toLocaleTimeString('id-ID')}` });
        saveRequestsToDB(requests);
        ...
        if (creator && creator.phone) kirimNotifikasiWA(creator.phone, `PERMINTAAN #${noSurat} DARI ${requests[idx].toko} TELAH SELESAI (DONE).`);
...
function kirimReject() {
  ...
  if (!alasan) { showNotif('MASUKKAN ALASAN PENOLAKAN!', 'warning'); return; }
  ...
  setTimeout(() => {
    ...
    if (idx !== -1) {
      ...
      if (!requests[idx].log) requests[idx].log = [];
      requests[idx].log.push({ action: `REJECT_${roleType}`, user: currentUser.fullName, notes: alasan, time: `${getFormattedDateDDMMYYYY()} ${new Date().toLocaleTimeString('id-ID')}` });
      saveRequestsToDB(requests);
      ...
      if (roleType === 'SERVICE') {
        ...
        if (creator && creator.phone) kirimNotifikasiWA(creator.phone, `PERMINTAAN #${noSurat} DITOLAK SERVICE. CATATAN: ${alasan}`);
      } else if (roleType === 'DM') {
        ...
        if (creator && creator.phone) kirimNotifikasiWA(creator.phone, `PERMINTAAN #${noSurat} DITOLAK DM PUSAT. CATATAN: ${alasan}`);
        const serviceUsers = users.filter(u => u.category === 'SERVICE' && u.area === requests[idx].area);
        serviceUsers.forEach(srv => {
          if (srv.phone) kirimNotifikasiWA(srv.phone, `PERMINTAAN #${noSurat} DARI ${requests[idx].toko} DITOLAK DM PUSAT. CATATAN: ${alasan}`);
        });
      }
      loadRiwayat();
      loadDashboard();
...
function lihatDetail(noSurat, fromDashboard = false) {
  ...
  if (req.status === 'PENDING') {
    if (role === 'SERVICE' || isAdminUser) {
      if (!req.serviceApprove) {
        actionButtons.push(`<button type="button" class="btnIcon btnApprove btnIconOnly" title="APPROVE" onclick="closeDetail(); approveService('${req.noSurat}');"><span class="material-symbols-rounded">check_circle</span></button>`);
        actionButtons.push(`<button type="button" class="btnIcon btnReject btnIconOnly" title="TOLAK" onclick="closeDetail(); tolakServiceModal('${req.noSurat}', 'SERVICE');"><span class="material-symbols-rounded">cancel</span></button>`);
      } else if (isAdminUser) {
        actionButtons.push(`<button type="button" class="btnIcon btnApprove btnIconOnly" title="APPROVE" onclick="closeDetail(); approveDM('${req.noSurat}');"><span class="material-symbols-rounded">check_circle</span></button>`);
        actionButtons.push(`<button type="button" class="btnIcon btnReject btnIconOnly" title="TOLAK" onclick="closeDetail(); tolakServiceModal('${req.noSurat}', 'DM');"><span class="material-symbols-rounded">cancel</span></button>`);
      }
    }
  ...
  if (isPdfVisible) {
    actionButtons.push(`<button type="button" class="btnIcon btnPdf btnIconOnly" title="CETAK PDF" onclick="closeDetail(); bukaPdfModal('${req.noSurat}');"><span class="material-symbols-rounded">picture_as_pdf</span></button>`);
  }

  if (req.status === 'APPROVE' && (role === 'SERVICE' || isAdminUser)) {
    actionButtons.push(`<button type="button" class="btnIcon btnDone btnIconOnly" title="SET DONE" onclick="closeDetail(); doneService('${req.noSurat}');"><span class="material-symbols-rounded">task_alt</span></button>`);
  }
  ...
  if (canCreatorEditDelete || canServiceEditDelete || canAdminEditDelete) {
    actionButtons.push(`<button type="button" class="btnIcon btnEdit btnIconOnly" title="EDIT" onclick="closeDetail(); editPermintaan('${req.noSurat}');"><span class="material-symbols-rounded">edit</span></button>`);
    actionButtons.push(`<button type="button" class="btnIcon btnDelete btnIconOnly" title="HAPUS" onclick="closeDetail(); hapusData('${req.noSurat}');"><span class="material-symbols-rounded">delete</span></button>`);
  }

  if (actionButtons.length > 0) {
    bottomActionsHtml = `<div class="popupDetailActions">${actionButtons.join('')}</div>`;
  }
...
function getActivePdfModel() { return appStorage.getItem(PDF_MODEL_KEY) || 'MODEL_1'; }
...
function tutupModalPdfModels() { document.getElementById('popupPdfModelsModal').style.display = 'none'; }
...
function renderFullPdfPreviewDocument(modelId) {
  ...
  let headerTitleHtml = `<div style="text-align: center; font-size: 20px; font-weight: 800; border-bottom: 2.5px solid #0f172a; padding-bottom: 6px; margin-bottom: 14px; letter-spacing: 0.5px; color: #0f172a; text-transform: uppercase;">PERMINTAAN TOKO</div>`;

  if (modelId === 'MODEL_2') {
    tableHeaderBg = '#334155';
    headerTitleHtml = `<div style="background: linear-gradient(135deg, #0284c7, #0369a1); color: #ffffff; padding: 12px 18px; border-radius: 10px; text-align: center; font-size: 20px; font-weight: 900; margin-bottom: 14px; letter-spacing: 1px; box-shadow: 0 4px 12px rgba(2,132,199,0.25);">PERMINTAAN TOKO</div>`;
  } else if (modelId === 'MODEL_3') {
    tableHeaderBg = '#0f172a';
    headerTitleHtml = `<div style="background: #0f172a; color: #fbbf24; padding: 14px 18px; border-radius: 8px; border-bottom: 4px solid #fbbf24; text-align: center; font-size: 21px; font-weight: 900; margin-bottom: 14px; letter-spacing: 1.5px; text-transform: uppercase;">PERMINTAAN TOKO</div>`;
  } else if (modelId === 'MODEL_4') {
    tableHeaderBg = '#059669';
    headerTitleHtml = `<div style="background: #059669; color: #ffffff; padding: 12px 18px; border-radius: 6px; text-align: center; font-size: 20px; font-weight: 900; margin-bottom: 14px; letter-spacing: 1px; border-left: 6px solid #047857;">PERMINTAAN TOKO</div>`;
  } else if (modelId === 'MODEL_5') {
    tableHeaderBg = '#7c3aed';
    headerTitleHtml = `<div style="background: linear-gradient(135deg, #7c3aed, #4c1d95); color: #ffffff; padding: 14px 18px; border-radius: 12px; text-align: center; font-size: 21px; font-weight: 900; margin-bottom: 14px; letter-spacing: 1.5px; box-shadow: 0 6px 18px rgba(124,58,237,0.3);">PERMINTAAN TOKO</div>`;
  }
  ...
  container.innerHTML = `
    <div style="background: #ffffff; color: #0f172a; width: 100%; max-width: 720px; margin: 0 auto; padding: 24px; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.15); font-family: Arial, sans-serif; box-sizing: border-box;">
      <div style="display: flex; justify-content: space-between; align-items: center; background: #f8fafc; border: 1px solid #cbd5e1; padding: 8px 12px; border-radius: 8px; margin-bottom: 16px;">
        <span style="font-size: 12px; font-weight: 800; color: ${m.color};"><span class="material-symbols-rounded" style="vertical-align: middle; font-size: 16px;">style</span> ${m.title}</span>
...
function tutupPdfModal() { document.getElementById('pdfModal').style.display = 'none'; }
function cetakDokumenPdf() { window.print(); }
...
function bukaTTD() {
  ...
  pushPopupHistoryState();
  setTimeout(() => { initCanvasTTD(); loadTTD(); }, 100);
}

function tutupTTD() {
  const modalTTD = document.getElementById('popupTTD');
  if (modalTTD) { modalTTD.classList.remove('show'); modalTTD.style.display = 'none'; }
}
...
function initCanvasTTD() {
  ...
  canvasTTD.onmousedown = null; canvasTTD.onmousemove = null; canvasTTD.onmouseup = null; canvasTTD.onmouseleave = null;
  canvasTTD.onmousedown = startDraw; canvasTTD.onmousemove = draw; canvasTTD.onmouseup = stopDraw; canvasTTD.onmouseleave = stopDraw;
...
function startDrawTouch(e) {
  ...
  ctxTTD.beginPath(); ctxTTD.moveTo(lastX, lastY);
}
...
function hapusTTD() { if (ctxTTD) ctxTTD.clearRect(0, 0, canvasTTD.width, canvasTTD.height); }
...
function simpanTTD() {
  showConfirm('SIMPAN TANDA TANGAN DIGITAL INI?', () => {
    ...
    if (currentUser.category === 'SERVICE') {
      ttdMap['SERVICE'] = png; ttdMap[`SERVICE_${currentUser.area}`] = png; ttdMap['HODS'] = png;
    }
...
function loadTTD() {
  ...
  if (data && ctxTTD) {
    const img = new Image();
    img.onload = () => { ctxTTD.clearRect(0, 0, canvasTTD.width, canvasTTD.height); ctxTTD.drawImage(img, 0, 0, canvasTTD.width, canvasTTD.height); };
    img.src = data;
  }
}
...
function bukaBantuan() {
  if (currentUser) isAdminChat = (currentUser.category === 'ADMIN' || (currentUser.username && currentUser.username.toUpperCase() === 'ADMIN') || (currentUser.category === 'SERVICE' && currentUser.area === 'TSM'));
  ...
  if (fastChatInterval) { clearInterval(fastChatInterval); fastChatInterval = null; }
...
function bukaRoomAdmin(room, user) {
  currentRoom = room; currentChatUser = user;
  const rooms = JSON.parse(appStorage.getItem(CHAT_ROOM_DB_KEY) || '[]');
...
function loadChatAdmin(room) {
  ...
  roomChats.forEach(c => {
    ...
    div.innerHTML = `<div class="chatText">${c.pesan}</div><div class="chatTime">${c.tanggal}</div>`;
...
function kirimPesanChat() {
  ...
  if (isAdminChat) {
    allChats.push({
      room: currentRoom, user: currentChatUser, pengirim: 'ADMIN', pesan,
    ...
    allChats.push({
      room, user: currentUser.username, pengirim: 'USER', pesan,
      tanggal: getFormattedDateDDMMYYYY() + ' ' + new Date().toLocaleTimeString('id-ID')
    });
    appStorage.setItem(CHAT_DB_KEY, JSON.stringify(allChats));
...
function cekUnreadNotif() {
  ...
  if (isAdminChat) {
    ...
    if (myRoom && myRoom.unreadUser > 0) { badge.textContent = myRoom.unreadUser > 99 ? '99+' : myRoom.unreadUser; badge.style.display = 'flex'; } 
    else { badge.style.display = 'none'; }
  }
}
...
function loadUsersManagement() {
  ...
  if (!tbody) return;
  let users = getUsersFromDB();
  if (!Array.isArray(users) || users.length === 0) { users = [...SEED_USERS]; saveUsersToDB(users); }
...
function bukaUserModal(userId = null) {
  if (typeof userId !== 'string' || userId.startsWith('[object')) userId = null;
...
function simpanUserData() {
  ...
  if (typeof editId !== 'string' || editId.startsWith('[object')) editId = '';
  ...
  if (!username || !password || !fullName) { showNotif('USERNAME, PASSWORD, DAN NAMA LENGKAP WAJIB DIISI!', 'warning'); return; }
  ...
  if (editId) {
    ...
    if (idx !== -1) {
      ...
      if (duplicateWithOtherUser) { showNotif(`USERNAME '${username}' SUDAH TERDAFTAR! GUNAKAN USERNAME LAIN.`, 'error'); return; }

      users[idx].username = username; users[idx].password = password; users[idx].fullName = fullName;
      users[idx].storeCode = storeCode; users[idx].phone = phone; users[idx].category = category; users[idx].area = area;
      saveUsersToDB(users);
      showNotif(`USER ${username} DIPERBARUI!`, 'info');
      tutupUserModal(); loadUsersManagement(); return;
    }
  }

  const deletedUserKeys = new Set((JSON.parse(appStorage.getItem(DELETED_USERS_KEY) || '[]') || []).filter(Boolean).map(v => String(v).trim()));
  const isDuplicate = users.some(u => {
    if (!u || !u.username) return false;
    const existingUsername = String(u.username).trim().toUpperCase();
    if (existingUsername !== username) return false;
    if (deletedUserKeys.has(String(u.id || '').trim()) || deletedUserKeys.has(existingUsername)) return false;
    return true;
  });

  if (isDuplicate) { showNotif(`USERNAME '${username}' SUDAH TERDAFTAR! GUNAKAN USERNAME LAIN.`, 'error'); return; }
  ...
  const newUser = {
    ...
    username, password, fullName, storeCode, phone, category, area, createdAt: getFormattedDateDDMMYYYY()
  };
  users.push(newUser);
  saveUsersToDB(users);
  showNotif(`USER ${fullName} (${username}) BERHASIL DISIMPAN!`, 'success');
  tutupUserModal(); loadUsersManagement();
}
...
function hapusUser(userId) {
  ...
  if (u.username.toUpperCase() === currentUser.username.toUpperCase()) {
    showNotif('TIDAK DAPAT MENGHAPUS AKUN AKTIF ANDA!', 'error'); return;
  }
...
function loadMasterDbTable() {
  ...
  if (search) {
    requests = requests.filter(r =>
      r.noSurat.toLowerCase().includes(search) || r.toko.toLowerCase().includes(search) || r.createdBy.toLowerCase().includes(search) || r.catatan.toLowerCase().includes(search) ||
      r.items.some(i => i.type.toLowerCase().includes(search) || i.seri.toLowerCase().includes(search) || i.barang.toLowerCase().includes(search))
    );
  }
  tbody.innerHTML = '';
  if (requests.length === 0) {
    tbody.innerHTML = `<tr><td colspan="9" style="text-align:center; padding:30px; color:var(--text-muted);">BELUM ADA DATA DI MASTER DATABASE.</td></tr>`; return;
  }
  ...
  requests.forEach(r => {
    ...
    tr.innerHTML = `
      ...
      <td style="text-align:center;"><button class="btnIcon btnDelete" onclick="hapusDataMaster('${r.noSurat}')" title="HAPUS PERMANEN"><span class="material-symbols-rounded">delete</span></button></td>
    `;
    tbody.appendChild(tr);
  });
}
...
function hapusDataMaster(noSurat) {
  showConfirm(`ADMIN: HAPUS PERMANEN DATA PERMINTAAN #${noSurat} DARI MASTER DATABASE?`, () => {
    ...
    setTimeout(() => {
      ...
      loadMasterDbTable(); loadRiwayat(); loadDashboard();
    }, 300);
  });
}
...
function downloadMasterExcel() {
  ...
  if (data.length === 0) { showNotif('TIDAK ADA DATA MASTER UNTUK DIEKSPOR!', 'warning'); return; }

  showLoading('MEMBUAT FILE EXCEL (.XLSX) MASTER LENGKAP...');
  setTimeout(() => {
    hideLoading();
    const rows = [];
    rows.push(['NO SURAT', 'TANGGAL', 'TOKO / PEMOHON', 'AREA', 'JENIS', 'TIPE BARANG', 'NO SERI', 'NO SERI DUS', 'PERMINTAAN', 'ALASAN', 'QTY', 'STATUS', 'CATATAN', 'LOG APPROVAL']);

    data.forEach(r => {
      const logStr = (r.log || []).map(l => `${l.action} by ${l.user} (${l.time})`).join(' | ');
      r.items.forEach(it => {
        rows.push([r.noSurat, r.tanggal, `${r.toko} (${r.createdBy})`, r.area, r.jenis, it.type, it.seri, it.dus || '', it.barang, it.alasan, it.qty, r.status, r.catatan || '', logStr]);
      });
    });
...
function prosesUploadExcelLookup(event) {
  ...
  if (typeof XLSX === 'undefined') { showNotif('MODUL SHEETJS UNTUK EXCEL BELUM TERMUAT!', 'error'); return; }
    ...
    try {
      ...
      jsonRows.forEach((row, idx) => {
        if (row && row.length >= 2) {
          ...
          if (colA && colB) { newLookup[colA] = colB; count++; }
        }
      });
      ...
      if (count > 0) {
        ...
        hideLoading(); showNotif('TIDAK ADA DATA VALID DENGAN 2 KOLOM (KOLOM A & KOLOM B)!', 'warning');
      }
    } catch (err) {
      hideLoading(); showNotif('GAGAL MEMBACA FILE EXCEL LOOKUP: ' + err.message, 'error');
    }
  };
...
function prosesBukaAkun() {
  ...
  if (menuTTD) menuTTD.style.display = (currentUser.category === 'SERVICE' || currentUser.category === 'DM') ? 'block' : 'none';

  const isToko = (currentUser.category === 'TOKO');
  const menuKelolaTokoAkun = document.getElementById('menuKelolaTokoAkun');
  if (menuKelolaTokoAkun) menuKelolaTokoAkun.style.display = isToko ? 'none' : 'block';

  const menuTambahTokoAkun = document.getElementById('menuTambahTokoAkun');
  if (menuTambahTokoAkun) menuTambahTokoAkun.style.display = isToko ? 'none' : 'block';
  ...
  if (popupAkun) {
    ...
    semuaElemen.forEach(el => {
      ...
      if (teks.includes('TAMBAH TOKO') && (el.tagName === 'BUTTON' || el.onclick)) {
        if (el.id !== 'menuKelolaTokoAkun' && el.id !== 'menuTambahTokoAkun') el.style.display = isToko ? 'none' : ''; 
      }
    });
  }
...
function tutupAkun() { document.getElementById('popupAkun').classList.remove('show'); }
...
function simpanAkun() {
  showConfirm('SIMPAN PERUBAHAN DATA AKUN?', () => {
    ...
    if (!nama) { showNotif('NAMA LENGKAP TIDAK BOLEH KOSONG!', 'warning'); return; }
...
function bukaModalTambahToko() {
  ...
  if (modalAreaText) modalAreaText.textContent = `${currentUser.area} (${AREA_MAP[currentUser.area] || currentUser.area})`;
  const inputEl = document.getElementById('inputNamaTokoBaru');
  if (inputEl) inputEl.value = '';
  loadDaftarTokoModal();
  const popup = document.getElementById('popupTambahToko');
  if (popup) { popup.style.display = 'flex'; popup.classList.add('show'); pushPopupHistoryState(); }
}
...
function loadDaftarTokoModal() {
  ...
  areaStores.forEach(s => {
    ...
    tr.innerHTML = `
      ...
      <td style="padding: 8px; text-align: center;"><button type="button" class="btnIcon btnDelete" onclick="hapusTokoCustom('${s.id}')" title="HAPUS TOKO"><span class="material-symbols-rounded">delete</span></button></td>
    `;
    tbody.appendChild(tr);
  });
}
...
function simpanTokoBaru() {
  ...
  if (!namaToko) { showNotif('NAMA TOKO TIDAK BOLEH KOSONG!', 'warning'); return; }

  const existingStores = getStoresFromDB();
  if (existingStores.some(s => s.fullName.toUpperCase() === namaToko && s.area === currentUser.area)) {
    showNotif(`TOKO '${namaToko}' SUDAH TERDAFTAR DI AREA ${currentUser.area}!`, 'warning'); return;
  }
  ...
  const localStores = JSON.parse(appStorage.getItem(STORES_DB_KEY) || '[]');
  const newStore = { id: newId, fullName: namaToko, area: currentUser.area, storeCode: generatedCode, createdBy: currentUser.fullName };
  ...
  if (!users.some(u => u.username.toUpperCase() === safeUsername)) {
    const newUserAcc = {
      id: newId, username: safeUsername, password: '123', fullName: namaToko,
      storeCode: generatedCode, phone: '-', category: 'TOKO', area: currentUser.area, createdAt: getFormattedDateDDMMYYYY()
    };
...
function downloadExcel() {
  const data = getAccessibleRequests();
  if (data.length === 0) { showNotif('TIDAK ADA DATA UNTUK DIEKSPOR!', 'warning'); return; }

  showLoading('MEMBUAT FILE EXCEL (.XLSX)...');
  setTimeout(() => {
    hideLoading();
    const rows = [];
    rows.push(['NO SURAT', 'TANGGAL', 'TOKO', 'AREA', 'JENIS PERMINTAAN', 'STATUS', 'NO', 'TYPE BARANG', 'NO SERI', 'DUS BARANG', 'PERMINTAAN DETAIL', 'ALASAN', 'QTY', 'PEMOHON', 'CATATAN']);

    data.forEach(r => {
      if (r.items && r.items.length > 0) {
        r.items.forEach((item, itemIdx) => {
          rows.push([r.noSurat, r.tanggal, r.toko, r.area, r.jenis, r.status, itemIdx + 1, item.type || '-', item.seri || '-', item.dus || '-', item.barang || '-', item.alasan || '-', item.qty || 1, r.createdBy, r.catatan || '']);
        });
      } else {
        rows.push([r.noSurat, r.tanggal, r.toko, r.area, r.jenis, r.status, 1, '-', '-', '-', '-', '-', 1, r.createdBy, r.catatan || '']);
      }
    });
    ...
    } else { showNotif('MODUL EXCEL (.XLSX) BELUM SIAP, PERIKSA KONEKSI INTERNET!', 'warning'); }
  }, 400);
}

// ======================================================================
// UI UTILITIES & IMAGE VIEWER
// ======================================================================
...
function zoomFoto(src) {
  currentZoom = 1;
  const img = document.getElementById('viewerImage');
  if (img) { img.src = src; img.style.transform = `scale(${currentZoom})`; }
  document.getElementById('imageViewer').style.display = 'flex';
  if (typeof pushPopupHistoryState === 'function') pushPopupHistoryState();
}
function tutupImageViewer() { document.getElementById('imageViewer').style.display = 'none'; }
...
function initDraggableElement(element, storageKey) {
  ...
  const savedPos = (window.localStorage) ? window.localStorage.getItem(storageKey) : null;
  if (savedPos) {
    try {
      ...
      if (typeof pos.left === 'number' && typeof pos.top === 'number') {
        ...
        el.style.position = 'fixed'; el.style.left = clampedX + 'px'; el.style.top = clampedY + 'px'; el.style.right = 'auto'; el.style.bottom = 'auto';
      }
    } catch (e) {}
  }
  ...
  function onPointerDown(e) {
    ...
    initialLeft = rect.left; initialTop = rect.top; isDragging = false;

    if (e.type === 'touchstart') { window.addEventListener('touchmove', onPointerMove, { passive: false }); window.addEventListener('touchend', onPointerUp); } 
    else { window.addEventListener('mousemove', onPointerMove); window.addEventListener('mouseup', onPointerUp); }
  }

  function onPointerMove(e) {
    const pointer = e.touches ? e.touches[0] : e;
    const deltaX = pointer.clientX - startX, deltaY = pointer.clientY - startY;

    if (!isDragging && (Math.abs(deltaX) > dragThreshold || Math.abs(deltaY) > dragThreshold)) { isDragging = true; el.classList.add('is-dragging'); }
    if (isDragging) {
      if (e.cancelable) e.preventDefault();
      let newLeft = initialLeft + deltaX, newTop = initialTop;
      const maxX = window.innerWidth - (el.offsetWidth || 48);
      newLeft = Math.max(0, Math.min(newLeft, maxX));
      el.style.position = 'fixed'; el.style.left = newLeft + 'px'; el.style.top = newTop + 'px'; el.style.right = 'auto'; el.style.bottom = 'auto';
    }
  }

  function onPointerUp(e) {
    window.removeEventListener('mousemove', onPointerMove); window.removeEventListener('mouseup', onPointerUp);
    window.removeEventListener('touchmove', onPointerMove); window.removeEventListener('touchend', onPointerUp);
    el.classList.remove('is-dragging');
    if (isDragging) {
      const rect = el.getBoundingClientRect();
      if (window.localStorage) window.localStorage.setItem(storageKey, JSON.stringify({ left: rect.left, top: rect.top }));
      const preventClick = function(evt) { evt.stopImmediatePropagation(); evt.preventDefault(); el.removeEventListener('click', preventClick, true); };
      el.addEventListener('click', preventClick, true);
    }
  }
  el.addEventListener('mousedown', onPointerDown); el.addEventListener('touchstart', onPointerDown, { passive: true });
}
...
(function() {
  ...
  styleTag.innerHTML = `body:not(:has(#dashboardPage.active)) #notifBellBtn, body:not(:has(#dashboardPage.active)) .notif-bell-btn, body:not(:has(#dashboardPage.active)) #helpButton, body:not(:has(#dashboardPage.active)) .helpButton { display: none !important; }`;
  ...
  showConfirm('YAKIN INGIN MENGHAPUS SEMUA DATA LOKAL & CACHE? (Aplikasi akan keluar dan dimuat ulang)', () => {
    ...
    setTimeout(async () => {
      try {
        ...
        hideLoading(); showNotif('TERJADI KESALAHAN SAAT MENGHAPUS DATA!', 'error');
      }
    }, 800);
  });
}
